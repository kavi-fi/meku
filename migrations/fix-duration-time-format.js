...
stream.on('data', function (program) {
  const fixedClassifications = _.map(program.classifications, (classification) => {
    if (classification.duration && classification.duration.length < 6){
      console.log(`SequenceId: ${program.sequenceId}  Duration: ${classification.duration}`)
      return classification
    }
    if (classification.duration && (classification.duration.length >= 6 && classification.duration.length < 8)){
      var m = /(?:(\d+)?:)(\d+):(\d+)$/.exec(classification.duration)
      if (m){
